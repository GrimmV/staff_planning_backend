...
class DataProcessor:
    ...
    def __init__(self, mas, clients, prio_assignments, distances, experience_log) -> None:
        self.mas = mas
    ...
    def create_day_dataset(self, clients, mas, date: datetime):
        ...
        clients_df, clients_dict = aggregate_client_features(
            clients, date, self.prio_assignments
        )
        mas_df, mas_dict = aggregate_ma_features(
            mas, self.distances, clients_dict, self.experience_log, date.strftime("%Y-%m-%d")
        )
