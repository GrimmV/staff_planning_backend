...
class Optimizer:
    ...
    def __init__(
        self, employees: pd.DataFrame, clients: pd.DataFrame, hard_constraints: Dict = {}, abnormality_model: AbnormalityModel = None
    ):
        ...
        self.forced_employees = hard_constraints.get("forced_employees", [])
        self.forced_clients = hard_constraints.get("forced_clients", [])
        self.banned_assignments = hard_constraints.get("banned_assignments", [])
        self.forced_assignments = hard_constraints.get("forced_assignments", [])
        
        self.unreachable_employees = set([self.employees.iloc[i]["id"] for i in range(len(self.employees))])
        self.unreachable_clients = set([self.clients.iloc[j]["id"] for j in range(len(self.clients))])
    ...
    def create_model(self):
        ...
        for i, emp in self.employees.iterrows():
            for j, client in self.clients.iterrows():
                if client["school"] in emp["timeToSchool"] and has_required_qualifications(
                    ...
                    self.unreachable_employees.discard(self.employees.iloc[i]["id"])
                    self.unreachable_clients.discard(self.clients.iloc[j]["id"])
        ...
        print(f"self.unreachable_employees: {self.unreachable_employees}")
        print(f"self.unreachable_clients: {self.unreachable_clients}")
        ...
        # Forced constraints
        for employee in self.forced_employees:
            emp_index = self.ma_id_index_mapping.get(employee, None)
            if emp_index is None:
                self.model += [False]
                continue
            self.model += [sum(self.assignments.get((emp_index, j), 0) for j in range(len(self.clients))) == 1]
        for client in self.forced_clients:
            client_index = self.client_id_index_mapping.get(client, None)
            if client_index is None:
                self.model += [False]
                continue
            self.model += [sum(self.assignments.get((i, client_index), 0) for i in range(len(self.employees))) == 1]
        for (employee, client) in self.banned_assignments:
            emp_index = self.ma_id_index_mapping.get(employee, None)
            client_index = self.client_id_index_mapping.get(client, None)
            if emp_index is None or client_index is None:
                self.model += [False]
                continue
            self.model += [self.assignments.get((emp_index, client_index), 0) == 0]
        for (employee, client) in self.forced_assignments:
            emp_index = self.ma_id_index_mapping.get(employee, None)
            client_index = self.client_id_index_mapping.get(client, None)
            if emp_index is None or client_index is None:
                self.model += [False]
    ...
    def process_results(self):
        ...
        store_dict["unassigned_employees"] = [{"id": unassigned_employee, "unreachable": unassigned_employee in self.unreachable_employees} for unassigned_employee in unassigned_employees]
        store_dict["unassigned_clients"] = [{"id": unassigned_client, "unreachable": unassigned_client in self.unreachable_clients} for unassigned_client in unassigned_clients]
        ...
        cols = ["timeToSchool", "cl_experience", "short_term_cl_experience", "school_experience", "priority", "availability_gap"]
    ...
    def process_employee_client_pair(self, emp_idx: int, client_idx: int) -> Dict:
        ...
        pair_data = {
            ...
            "short_term_cl_experience": emp["short_term_cl_experience"].get(
                client["id"], 0
            ),
        ...
        label = self.abnormality_model.predict(np.array([list(pair_data.values())]))

        pair_data["abnormality_score"] = int(label[0])
        pair_data["ma"] = emp["id"]
